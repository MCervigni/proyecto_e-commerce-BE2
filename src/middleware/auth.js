...
export const authenticate = passport.authenticate("current", {
  session: false,
});
  ...
  return (req, res, next) => {
    if (!req.user || (roles.length && !roles.includes(req.user.role))) {
      return res
        ...
        .json({
          error:
            "NO AUTORIZADO - No tiene privilegios para acceder a este recurso",
        });
    }
    next();
  };
...
export const isCartOwner = async (req, res, next) => {
  ...
  if (!cart.user)
    return res.status(403).json({ error: "Carrito sin usuario asignado" });
  if (cart.user !== req.user.email) {
    return res
      .status(403)
      .json({ error: "NO AUTORIZADO - El carrito corresponde a otro usuario" });
  }
  next();
};
